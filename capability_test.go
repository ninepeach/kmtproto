...
func TestCapabilitySerialization(t *testing.T) {
	...
	helloPayload := HelloPayload{
		...
		Capabilities: []CapabilityOffer{
			{Name: "state-sync", Versions: []uint16{1, 2}, Required: true},
	...
	welcomePayload := WelcomePayload{
		Mode:                 WelcomeModeNew,
		ServerTime:           123,
		AcceptedCapabilities: []NegotiatedCapability{{Name: "state-sync", Version: 2}},
	}
...
func TestCapabilityNegotiationIntersection(t *testing.T) {
	registry, err := NewCapabilityRegistry([]CapabilitySpec{
		{Name: "state-sync", Versions: []uint16{1, 3}},
		{Name: "presence", Versions: []uint16{1}},
	}, DefaultLimits())
	if err != nil {
		t.Fatal(err)
	...
	offers := []CapabilityOffer{
		{Name: "state-sync", Versions: []uint16{3, 2, 1}},
		{Name: "compression.zstd", Versions: []uint16{1}},
		{Name: "presence", Versions: []uint16{1}, Required: true},
	}
...
func TestInvalidCapabilityRejected(t *testing.T) {
	tests := []struct {
		...
		{name: "empty versions", offers: []CapabilityOffer{{Name: "state-sync"}}},
		{name: "zero version", offers: []CapabilityOffer{{Name: "state-sync", Versions: []uint16{0}}}},
		{name: "duplicate version", offers: []CapabilityOffer{{Name: "state-sync", Versions: []uint16{1, 1}}}},
		{name: "duplicate offer", offers: []CapabilityOffer{{Name: "state-sync", Versions: []uint16{1}}, {Name: "state-sync", Versions: []uint16{2}}}},
	}
...
func TestCapabilityHandshakeStoresSessionState(t *testing.T) {
	...
	registry, err := NewCapabilityRegistry([]CapabilitySpec{{Name: "state-sync", Versions: []uint16{1, 2}}}, limits)
	if err != nil {
		t.Fatal(err)
	}
	...
	server, err := NewServer(serverConfig, sessions, NewMemoryDedupStore(clock, serverConfig.DedupTTL), replay, replay, &recordingApp{})
	if err != nil {
		t.Fatal(err)
	}
	...
	if client.CapabilityEnabled("state-sync") {
		t.Fatal("client exposed advertised capability before WELCOME")
	}
	if _, ok := client.CapabilityVersion("state-sync"); ok {
	...
	if connection.CapabilityEnabled("state-sync") {
	...
	want := []NegotiatedCapability{{Name: "state-sync", Version: 2}}
	...
	if version, ok := client.CapabilityVersion("state-sync"); !ok || version != 2 {
		t.Fatalf("client capability query mismatch: version=%d enabled=%v", version, ok)
	}
	if version, ok := connection.CapabilityVersion("state-sync"); !ok || version != 2 {
	...
	if storedAgain.Capabilities.List()[0].Version != 2 {
...
func TestRequiredCapabilityFailureDoesNotCreateSession(t *testing.T) {
	...
	server, err := NewServer(config, sessions, NewMemoryDedupStore(clock, config.DedupTTL), replay, replay, &recordingApp{})
	if err != nil {
		t.Fatal(err)
	}
	outbound := NewOutboundQueue()
	hello := Envelope{V: WireVersionV2, Type: FrameHello, Payload: mustPayload(HelloPayload{Capabilities: []CapabilityOffer{{Name: "state-sync", Versions: []uint16{1}, Required: true}}})}
	...
	_, err = registry.Negotiate([]CapabilityOffer{{Name: "state-sync", Versions: []uint16{1}, Required: true}}, DefaultLimits())
